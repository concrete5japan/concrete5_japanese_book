...
<?php	 if ($enablePermissions) { ?>
	...
	ccm_permissionLaunchDialog = function(link) {
		jQuery.fn.dialog.open({
			...
			href: '<?php	echo REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/dialogs/basic_workflow?wfID=<?php	echo $workflow->getWorkflowID()?>&pkID=' + $(link).attr('data-pkID') + '&paID=' + $(link).attr('data-paID'),
			modal: false,
			width: 500,
			height: 380
		});		
