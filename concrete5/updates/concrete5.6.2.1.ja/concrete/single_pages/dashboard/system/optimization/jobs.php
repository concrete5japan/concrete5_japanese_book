...
<p><?php	echo t('To run all the jobs in the <a href="%s">%s</a> Job Set, schedule this URL using cron or a similar system:', $this->url('/dashboard/system/optimization/jobs', 'edit_set', $djs->getJobSetID()), tc('JobSetName', $djs->getJobSetName()))?></p>
...
$(function() {
	...
	$('a.ccm-automate-job-instructions').on('click', $("#ccm-jobs-list"), function() {
		...
		$('#jd' + $(this).attr("data-jID")).jqdialog({
			...
			width: 650,
			modal: true,
			title: '<?php	echo t('Automation Instructions')?>'
		});
	});
