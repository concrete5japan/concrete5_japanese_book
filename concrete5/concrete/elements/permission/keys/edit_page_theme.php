...
<div class="clearfix">
	...
	<ul class="theme-list inputs-list" <?php	 if ($assignment->getThemesAllowedPermission() != 'C') { ?>style="display: none"<?php	 } ?>>
		<?php	 foreach($themes as $ct) { ?>
			<li><label><input type="checkbox" name="ptIDInclude[<?php	echo $entity->getAccessEntityID()?>][]" value="<?php	echo $ct->getThemeID()?>" <?php	 if (in_array($ct->getThemeID(), $assignment->getThemesAllowedArray()) || $assignment->getThemesAllowedPermission() == 'A') { ?> checked="checked" <?php	 } ?> /> <span><?php	echo $ct->getThemeName()?></span></label></li>
		<?php	 } ?>
	</ul>
	</div>
</div>
...
<div class="clearfix">
	...
	<ul class="theme-list inputs-list" <?php	 if ($assignment->getThemesAllowedPermission() != 'C') { ?>style="display: none"<?php	 } ?>>
		<?php	 foreach($themes as $ct) { ?>
			<li><label><input type="checkbox" name="ptIDExclude[<?php	echo $entity->getAccessEntityID()?>][]" value="<?php	echo $ct->getThemeID()?>" <?php	 if (in_array($ct->getThemeID(), $assignment->getThemesAllowedArray()) || $assignment->getThemesAllowedPermission() == 'N') { ?> checked="checked" <?php	 } ?> /> <span><?php	echo $ct->getThemeName()?></span></label></li>
		<?php	 } ?>
	</ul>
	</div>
</div>
