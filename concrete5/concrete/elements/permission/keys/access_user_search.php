...
<div class="clearfix">
	...
	<ul class="inputs-list" <?php	 if ($assignment->getGroupsAllowedPermission() != 'C') { ?>style="display: none"<?php	 } ?>>
		<?php	 foreach($groups as $g) { ?>
			<li><label><input type="checkbox" name="gIDInclude[<?php	echo $entity->getAccessEntityID()?>][]" value="<?php	echo $g->getGroupID()?>" <?php	 if (in_array($g->getGroupID(), $assignment->getGroupsAllowedArray())) { ?> checked="checked" <?php	 } ?> /> <span><?php	echo $g->getGroupName()?></span></label></li>
		<?php	 } ?>
	</ul>
	</div>
</div>
